...
function aipim_enqueue_ui_scripts_games() {
    if( !is_game() ) return;

    wp_enqueue_script( 'aipim-favs-js', get_template_directory_uri().'/assets/javascript/favs.js', array( 'jquery' ), false, true );
    ...
    wp_localize_script( 'aipim-favs-js', 'my_ajax_object', array(
      'ajax_url' => $fav_ajax_url,
      'security' => wp_create_nonce('fav-like'),
...
// games favs script creation for casinos
function aipim_enqueue_ui_scripts_casinos() {
    if ( !is_casino() ) return;
    ...
    if ( is_user_logged_in() ) {
      ...
      $game_check = ",".get_the_ID();
      if (stristr($favorite_casinos, $game_check) === FALSE) $fav_action = "add-favorite-casino";
    }
    ...
    wp_localize_script( 'aipim-favs-js', 'my_ajax_object', array(
      ...
      'fav_game_id' => get_the_ID(),
      'fav_user_id' => $fav_user_id,
      'fav_user_favgames' => $favorite_casinos
    ));
}

add_action( 'wp_enqueue_scripts', 'aipim_enqueue_ui_scripts_games');
add_action( 'wp_enqueue_scripts', 'aipim_enqueue_ui_scripts_casinos');



// ajax handler
function aipim_favs_add() {
    if ( ! check_ajax_referer( 'fav-like', 'security' ) ) {
...
add_action( 'wp_ajax_fav_like', 'aipim_favs_add' );
add_action( 'wp_ajax_nopriv_fav_like', 'aipim_favs_add' );
