...
function aipim_shortcode_player_test( $atts ) {
  global $current_user;
  $ret = "";
  ...
  $survery_already_done = get_field('hizo_encuesta', "user_".$current_user->ID);
  $user_prtest_questions = aipim_short_player_test_questions();


  echo '<style type="text/css">.mb-3:first-child{display:none;} .betizen-features img{max-width:85%;}</style>';
    ...
    ?>
      <section class="hero hero-done">
        ...
        <?php
          ...
          if (in_array("under-aged", $veredicts)){
            ...
            if (in_array("money-player", $veredicts)){
              ...
              if (in_array("unhealthy-player", $veredicts)){
                  ...
                  $veredict_texts .= __("<br>While we process your doubts with the utmost discretion, we recommend you read the read", "aipim")." <a href='".__("/en/articles/", "aipim")."'>".__("articles on gambling", "aipim")."</a> ".__("written by our experts.", "aipim");
          ...
          if ($veredict_show_contact) $veredict_texts .= "<br><br><a class='button button-brand btn-lg mb-5 mb-lg-2 hero-button btn-xs-block' href='".__("/en/contact/", "aipim")."'>".__("Get in touch (it's free)", "aipim")."</a>";
...
// Return the array of questions for the user profile test
function aipim_short_player_test_questions(){
  ...
  return $user_prtest_questions;
}


add_shortcode( 'playertest', 'aipim_shortcode_player_test' );
